define([
    ...
    Helpers.UpdateHtmlFontAttributes = function(
        ...
        var fontLoadCallback = function() {
            ...
            for (var i = 0; i < $textblocks.length; i++) {
                ...
                var lineHeightAttr = ele.getAttribute(
                    "data-original-line-height"
                );
                var originalLineHeight = lineHeightAttr
                    ? Number(lineHeightAttr)
                    : 0;
                if (originalLineHeight) {
                    $(ele).css(
                        "line-height",
                        originalLineHeight * factor + "px"
                    );
                }
    ...
    Helpers.RemoveFromString = function(str, toRemove) {
        ...
        return (
            str.substring(0, startIx) + str.substring(startIx + toRemove.length)
        );
    };
    ...
    Helpers.setStyles = function(styles, $element) {
        ...
        var stylingGlobal = "";
        var stylings = [];
        var elementIsDocument =
            $element && $element.createTextNode ? true : false;
                    ...
                    convertToDOMRange: function(textRange, document) {
                        var adoptBoundary = function(
                            ...
                            if (
                                ...
                                // element
                                domRange[
                                    bStart ? "setStartBefore" : "setEndBefore"
                                ](cursorNode);
                            }
                ...
                document.caretRangeFromPoint = function caretRangeFromPoint(
                    ...
                    for (var iy = IYDepth; iy; iy = iy - 4) {
                        try {
                            ...
                            return TextRangeUtils.convertToDOMRange(
                                tr,
                                document
                            );
                        } catch (ex) {}
