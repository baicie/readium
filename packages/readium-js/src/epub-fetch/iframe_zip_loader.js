    ...
    var zipIframeLoader = function (
        ...
        this._loadIframeWithDocument = function (
            ...
            if (isBlobHandled) {
                ...
                if (
                    attachedData.spineItem.media_type &&
                    attachedData.spineItem.media_type.length
                ) {
            ...
            iframe.onload = function () {
                ...
                if (iframe.contentWindow.frames) {
                    for (
                        var i = 0;
                        i < iframe.contentWindow.frames.length;
                        i++
                    ) {
                        ...
                        if (!childSrc) {
                            if (
                                child_iframe.frameElement.localName == "iframe"
                            ) {
                        ...
                        var contentDocumentPathRelativeToPackage =
                            attachedData.spineItem.href;
        ...
        function fetchContentDocument(src, callback) {
            fetchHtmlAsText(src, function (contentDocumentHtml) {
                ...
                if (_contentDocumentTextPreprocessor) {
                    contentDocumentHtml = _contentDocumentTextPreprocessor(
                        src,
                        contentDocumentHtml
                    );
                }
